...
export class AuthService {
  ...
  async register(data: { email: string; name: string; password: string }) {
    ...
    if (existingUser) {
      if (existingUser.isEmailVerified) {
        ...
        return {
          message:
            'Tasdiqlash kodi qayta yuborildi. Iltimos, emailingizni tekshiring.',
    ...
    return {
      message:
        'Ro`yxatdan o`tdingiz. Iltimos, emailingizga yuborilgan 6 xonali kodni kiriting.',
      userId: user.id,
    };
  }
  ...
  async resendVerificationCode(email: string) {
    ...
    return { message: 'Yangi tasdiqlash kodi emailingizga yuborildi.' };
  }
  // ==========================================
  ...
  // ==========================================
  async googleLogin(
    googleUser: { email: string; name: string; googleId?: string } | undefined,
  ) {
    if (!googleUser)
      throw new BadRequestException('Google ma`lumotlari topilmadi');
  ...
  async forgotPassword(email: string) {
    ...
    return {
      message: 'Parolni tiklash uchun emailingizga 6 xonali kod yuborildi.',
    };
  }
  ...
  async resetPassword(data: {
    ...
    return {
      message:
        'Parolingiz muvaffaqiyatli o`zgartirildi. Endi yangi parol bilan login qilishingiz mumkin.',
    };
  }
  ...
  async refreshTokens(rt: string) {
    ...
    if (!rtMatches) throw new UnauthorizedException('Ruxsat etilmagan');
